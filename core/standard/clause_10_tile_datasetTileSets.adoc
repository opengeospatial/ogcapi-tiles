...
This requirements class defines a mechanism to retrieve a list of tilesets for a dataset (a.k.a., a root resource) which may contain multiple geospatial data resources. It is used in conjunction with the  Requirements Class "Tilesets List" (<<rc_tileSets-list>>) that specifies the response to this GET request.
...
The landing page provides links to start exploring the resources offered by the Web API. It mainly consists of a list of links to root resources. This requirements class requires including in the landing page one or more link(s) to list(s) of tilesets available for the dataset.
