...
For a case of multidimensional regular grids there is a need for a format able to store multidimensional data in a natural way. The NetCDF is one of the most popular formats for scientific data that is able store multi-dimensional arrays of data values. It is defined by the UCAR-Unidata.
...
NOTE: The support of Mapbox Vector Tiles does not make the OGC API dependent on the Mapbox Vector Tile Specification. The support of Mapbox is completely optional in a Web API. This requirements class is completely independent of the version of Mapbox and its future evolution.
