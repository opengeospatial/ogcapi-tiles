...
|Collection^6^ |`/collections/{collectionId}`
|Tiling Schemes |`/tileMatrixSets`
|Tiling Scheme |`/tileMatrixSets/{tileMatrixSetId}`
2+|Tiles
|Feature tiles^3^ description |`/collections/{collectionId}/tiles`
|Feature tiles^3^ description in one tile matrix set^2^ |`/collections/{collectionId}/tiles/{tileMatrixSetId}`
|Feature tile^3^ |`/collections/{collectionId}/tiles/{tileMatrixSetId}/{tileMatrix}/{tileRow}/{tileCol}`
|Feature tiles^3^ description (geospatial resources^1^) |`/tiles`
|Feature tile^3^ |`/collections/{collectionId}/tiles/{tileMatrixSetId}/{tileMatrix}/{tileRow}/{tileCol}`
|Feature tile^3^ (geospatial resources^1^) |`/tiles/{tileMatrixSetId}/{tileMatrix}/{tileRow}/{tileCol}`
2+|Maps
|Maps description^4^ |`/collections/{collectionId}/map`
|Maps description (geospatial resources^1^)^4^ |`/map`
2+|Map tiles
...
^4^: Specified in the OGC API - Maps Part 1, core
^5^: Specified in the OGC API - Common Part 1, core
^6^: Specified in the OGC API - Common Part 2, collections
