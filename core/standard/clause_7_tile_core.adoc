...
This building block stays flexible and does not require to implement OWS Common, allowing for other API architectures to adopt it. However, a server integrating some of the OGC APIs is expected to implement OGC API Common. If so, in practice, this means that the landing page and the conformance page follow OGC API - Common core and collections requirement classes will be used.
...
If the server declares also conformity to OGC API - Common; it has to consider the OGC API - Common requirements for declaring conformance, i.e. the use of a the conformance page. In the JSON format the links follow the link schema defined in the OGC API - Common. Below is an example fragment of a conformance information page of an API conformant to OGC API - Common and OGC API - Tiles.
...
This draft specification does not specify how geospatial resources are exposed and if they have to be retrievable as geospatial data (e.g. feature items). For example OGC API features includes de definition of collections and each collection is available in the /collections/{collectionId} path. OGC API common provides a similar mechanism. Other paths in the API could also give access to geospatial resources.

NOTE: The concept of geospatial resource path substitutes the concept of "layer" in WMTS 1.0.
...
For example, the OGC API Features return a list of links that could include resource views for each geospatial resource in the /collections/{collectionId} path. OGC API common provides a similar mechanism. In the JSON response, the array `links` is the place for adding a resource reference to the 'tiles' description.
...
This standard does not specify the need for any parameter in the GET request.
