...
The landing page provides links to start exploring the resources offered by the Web API. It mainly consists of a list of links. New links for TileMatrixSets on top of the common ones are introduced.

On the landing page, in JSON format, the links follow the link schema defined in OGC API - Common. The following is an example fragment of the response to an OGC API - Tiles landing page.
