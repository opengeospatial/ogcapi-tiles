...
^|C |It is also possible to use TileMatrixSet no available in a registry. Int this case the API SHOULD provide a definition of the TileMatrixSet as resolvable URI in an endpoint following this template: "/tileMatrixSets/{tileMatrixSetsId}" (where {tileMatrixSetsId} is a unique identifier of the TileMatrixSet in the service)
^|D |When the API provides one or more definitions of TileMatrixSets as resolvable URI in "/tileMatrixSets/{tileMatrixSetsId}" the "/tileMatrixSets
" endpoint SHOULD respond with the list of TileMatrixSets defined by the API
